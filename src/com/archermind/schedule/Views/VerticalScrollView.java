...
public class VerticalScrollView extends ViewGroup {
			...
			if (i == 0) {
				getChildAt(i).measure(
						MeasureSpec.makeMeasureSpec(getMeasuredWidth(),
								MeasureSpec.EXACTLY),
						MeasureSpec.makeMeasureSpec((int) (pageHeight * .75),
								MeasureSpec.EXACTLY));
			} else {
				getChildAt(i).measure(
						MeasureSpec.makeMeasureSpec(getMeasuredWidth(),
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int i = 0; i < count; i++) {
			...
			if (child.getVisibility() != View.GONE) {
				if (i == 0) {
					child.getHeight();
					child.layout(0, measuredHeight, right - left,
							measuredHeight + (int) (pageHeight * .75));
					measuredHeight += (pageHeight * .75);
				} else {
					height = pageHeight
							* (int) Math.ceil((double) child
									.getMeasuredHeight() / (double) pageHeight);
	...
	// final int count = getChildCount();
	// if (getScrollY() < 0 || getScrollY() + pageHeight > getChildAt(count -
	// 1).getBottom()) {
	// deltaY /= 2;
	// }
	//
	...
	// child.getTop(), child.getBottom() - getHeight());
	// invalidate();
	// break;
	// } else if(child.getBottom() > getScrollY() && child.getBottom() <
