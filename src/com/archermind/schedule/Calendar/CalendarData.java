...
public class CalendarData {
    ...
    public CalendarData(Context context, int jumpMonth, int jumpYear, int year_c, int month_c,
    ...
    private synchronized void getweek(int year, int month, int flagType) {
        ...
        SharedPreferences sharedPreferences = mContext.getSharedPreferences(ScheduleData.LUNARDATE,
        ...
        if (!TextUtils.isEmpty(res)) {// 已经存在，不需要计算
            ...
            isExist = true;
            dayNumber = res.split(",");
            ScheduleApplication.LogD(CalendarData.class, " 默认的阴历: " + res + "daynum number is "
                    + dayNumber.length);
        ...
        if (!isExist) {
            ...
            spedit.commit();
            ScheduleApplication.LogD(CalendarData.class, "写入默认的阴历:" + date + " 数据是： " + data);
        } else {
    ...
    public synchronized List<ScheduleData> getMonthSchedule(int year, int month) {
        ...
        for (i = 1; i <= days; i++) {
            ...
            cursor = database
                    .queryIsMarkWithDay(starTimeInMillis, dayOfYear, dayOfMonth, dayOfWeek);
            ...
            while (cursor.moveToNext()) {
                ...
                scheduledata.content = cursor.getString(cursor
                        .getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_CONTENT));
                scheduledata.time = cursor.getLong(cursor
                ...
                scheduledata.notice_flag = cursor.getInt(cursor
