...
public class LoginScreen extends Screen implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		login_username
				.setText(ServiceManager.getSPUserInfo(UserInfoData.EMAIL));
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (msg != null && msg.obj != null) {
					...
					ScheduleApplication.LogD(LoginScreen.class, "retValue = "
							+ retValue);
					if (retValue.contains("user_id")) {
						ServiceManager.ToastShow("登录成功");
						RegisterScreen.writeUserinfo(retValue,
								HttpUtils.GetCookie());
						...
						if (ServiceManager
								.getSPUserInfo(UserInfoData.TEL).equals("")||ServiceManager
								.getSPUserInfo(UserInfoData.TEL).equals("null")) {
							ServiceManager.ToastShow("您的帐号尚未绑定手机号,请进行绑定!");
							startActivity(new Intent(LoginScreen.this,
									TelephoneBindScreen.class));
						} else if (!imsi.equals(ServiceManager
								.getSPUserInfo(UserInfoData.IMSI))) /* IMSI发生变化 */
						{
	...
	private void loginSina() {
		...
		weibo.setupConsumerConfig("2178333014",
				"9027b1d8aaf67311bb41c0edf0147acd");
	...
	private void loginRenren() {
		...
		renren = new Renren(API_RENREN_KEY, SECRET_RENREN_KEY, APP_RENREN_ID,
				LoginScreen.this);
			...
			if (resultCode == OAuthV1AuthorizeWebView.RESULT_CODE) {
				...
				try {
					...
					UserAPI userAPI = new UserAPI(
							OAuthConstants.OAUTH_VERSION_1);
	...
	public void binAccount(String uid, int tag) {
		...
		String isBin = ServiceManager.getServerInterface().Bin_login(
				Integer.toString(tag), uid);
		...
		if (NetworkUtils.getNetworkState(this) != NetworkUtils.NETWORN_NONE) {
			...
			new Thread() {
				public void run() {
					...
					String ret = ServiceManager.getServerInterface().login(
							username, passwordCrypt, imsi);
					ScheduleApplication.LogD(LoginScreen.class, "run() ret = "
							+ ret);
