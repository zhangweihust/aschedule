...
public class ScheduleAddScreen extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		scheduleType.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(ScheduleAddScreen.this, ScheduleTypeScreen.class);
		...
		dateText.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new TimePickerDialog(ScheduleAddScreen.this, new OnTimeSetListener() {
		...
		scheduleSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(TextUtils.isEmpty(scheduleText.getText().toString())){
					...
					new AlertDialog.Builder(ScheduleAddScreen.this).setTitle("输入日程").setMessage("日程信息不能为空").setPositiveButton("确认", null).show();
				}else{
					//将日程信息保存
					...
					Intent intent = new Intent();
					intent.setClass(ScheduleAddScreen.this, ScheduleInfoScreen.class);
	...
	public void setScheduleDateTag(int remindID, String year, String month, String day,int scheduleID){
		...
		if(remindID >= 0 && remindID <= 3){
			...
			//每周重复（从设置日程的这天（星期几），接下来的每周的这一天都要标记）
			...
			//每月重复（从设置日程的这天（几月几后），接下来的每月的这一天都要标记）
			...
			//每年重复（从设置日程的这天（哪一年几月几号），接下来的每年的这一天都要标记）
	...
	public String getScheduleDate() {
		...
		if(intent.getStringArrayListExtra("scheduleDate") != null){
			//从CalendarActivity中传来的值（包含年于日信息）
	...
	public String getLunarDay(int year, int month, int day) {
		...
		// {由于在取得阳历对应的阴历日期时，如果阳历日期对应的阴历日期为“初一“，就被设置成了月份（如：四月，五月。。。等）}，
		// 所以在此就要判断得到的阴历日期是否为月份，如果是月份就设置为“初一“
