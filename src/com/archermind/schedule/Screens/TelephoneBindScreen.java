...
public class TelephoneBindScreen extends Screen implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ServiceManager.getBindFlag()) {
			...
			String promptstr = "您的帐号 "
					+ ServiceManager.getSPUserInfo(UserInfoData.EMAIL)
					+ " 已经与手机号 "
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
					...
					case TELEPHONE_BIND_ALREADY :
						Toast.makeText(TelephoneBindScreen.this, "这个手机号已经绑定!",
					...
					if (inputstr.matches("^(\\+86)?1[3|4|5|8][0-9]\\d{8}$")) {
						...
						new Thread() {
							public void run() {
								...
								int is_tel_bindret = ServiceManager
										.getServerInterface().is_tel_bind(
												(String.valueOf(ServiceManager
														.getUserId())),
												requestTel);
								...
								if (0 == is_tel_bindret) {
									String ret = ServiceManager
											.getServerInterface().sendSMS(
													SCHEDULE_APP_ID,
													requestTel, "default");
									ScheduleApplication.LogD(
											TelephoneBindScreen.class, "ret = "
													+ ret);
			...
			case R.id.telephone_bind_verification_btn :
				...
				if (!inputstr.equals("")) {
					...
					if (canBingFlag) {
						...
						new Thread() {
							public void run() {
								int ret = ServiceManager.getServerInterface()
										.checkSMS(
												SCHEDULE_APP_ID,
												verficationcode,
												smsID,
								...
								if (ret == 0) {
									...
									ServiceManager.setSPUserInfo(
											UserInfoData.TEL, tel);
									ServiceManager.setSPUserInfo(
											UserInfoData.IMSI, finalimsi);
