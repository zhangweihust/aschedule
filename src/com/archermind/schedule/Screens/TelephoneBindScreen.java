                ...
                if (inputstr.matches("^(\\+86)?1[3|4|5|8][0-9]\\d{8}$")) {
                    ...
                    new Thread() {
                        public void run() {
                            ...
                            int is_tel_bindret = ServiceManager.getServerInterface().is_tel_bind(
                                    (String.valueOf(ServiceManager.getUserId())), requestTel);
                            ScheduleApplication.LogD(TelephoneBindScreen.class, "is_tel_bindret = " + is_tel_bindret);
                            if (0 == is_tel_bindret) {
                                String ret = ServiceManager.getServerInterface().sendSMS(SCHEDULE_APP_ID, requestTel,
                                        "default");
        ...
        case R.id.telephone_bind_verification_btn:
            ...
            if (!inputstr.equals("")) {
                ...
                if (canBingFlag) {
                    ...
                    new Thread() {
                        public void run() {
                            int ret = ServiceManager.getServerInterface().checkSMS(SCHEDULE_APP_ID, verficationcode,
                                    smsID, "bind", String.valueOf(ServiceManager.getUserId()), tel, finalimsi);
                            if (ret == 0) {
                                handler.sendEmptyMessage(TELEPHONE_BIND_SUCCESS);
                                eventService.onUpdateEvent(new EventArgs(EventTypes.TELEPHONE_BIND_SUCCESS));
