...
public class EditScheduleScreen extends Screen implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TAG,
                " detail startTime = "
                        + DateTimeUtils.time2String("yyyy-MM-dd-HH-mm", scheduleTime));
    ...
    public void init() {
        ...
        alarmPopwindow.setRemind(mRemind, remindCycle, weekType, mStageRemind, startTime, endTime);
    ...
    public void readDb() {
        ...
        if (c.moveToFirst()) {
            scheduleText = c.getString(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_CONTENT));

            scheduleTime = c.getLong(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_START_TIME));

            mShare = c.getInt(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_SHARE)) == 1;

            mType = c.getInt(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_TYPE));

            mRemind = c.getInt(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_NOTICE_FLAG)) == 1;
            ...
            weekType = c.getString(c.getColumnIndex(DatabaseHelper.COLUMN_SCHEDULE_NOTICE_WEEK));
            ...
            endTime = Long.parseLong(c.getString(c
    ...
    public void setDisplayTime(long displaytime) {
        ...
        String time = amORpm + " " + DateTimeUtils.time2String("hh:mm", displaytime);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == backBtn.getId()) {
            ...
            if ("".equals(scheduleText.toString().trim())) {
                Toast.makeText(EditScheduleScreen.this, "内容不能为空", Toast.LENGTH_SHORT).show();
            } else {
                // 删除之前的闹钟
                // cancelAlarm();
            ...
            if (mShare == false && ServiceManager.getUserId() == 0) {
                Toast.makeText(EditScheduleScreen.this, "请登录以后再分享日程", Toast.LENGTH_SHORT).show();
                return;
            }
            // 判断时间是否可以分享，如果大于当前时间则可以分享
            if (mSelectTime < System.currentTimeMillis()) {
                // 提示不能分享；
                Toast.makeText(EditScheduleScreen.this, "不能分享过去事件", Toast.LENGTH_SHORT).show();
            } else {
                if (mShare == false) {
                    mShare = true;
                    shareImg.setImageResource(R.drawable.schedule_new_share_select);
    ...
    public void updateScheduleToDb() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (DatabaseHelper.SCHEDULE_NOTICE_PERIOD_MODE_MONTH.equals(remindCycle)) {
                    monthday = DateTimeUtils.time2String("d", startTime);
                }

                cv.put(DatabaseHelper.COLUMN_SCHEDULE_NOTICE_MONTHDAY, monthday);
                if (DatabaseHelper.SCHEDULE_NOTICE_PERIOD_MODE_YEAR.equals(remindCycle)) {
