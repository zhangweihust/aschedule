...
public class DatabaseManager {
	...
	public int save(ScheduleVO scheduleVO) {
		...
		try {
			...
			Cursor cursor = database.rawQuery(
					"select max(scheduleID) from schedule_test", null);
	...
	public ScheduleVO getScheduleByID(int scheduleID) {
		// dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		Cursor cursor = database.query("schedule_test", new String[] {
				"scheduleID", "scheduleTypeID", "remindID", "scheduleContent",
		...
		if (cursor.moveToFirst()) {
			...
			int scheduleTypeID = cursor.getInt(cursor
					.getColumnIndex("scheduleTypeID"));
			int remindID = cursor.getInt(cursor.getColumnIndex("remindID"));
			String scheduleContent = cursor.getString(cursor
					.getColumnIndex("scheduleContent"));
			String scheduleDate = cursor.getString(cursor
					.getColumnIndex("scheduleDate"));
			cursor.close();
			return new ScheduleVO(schID, scheduleTypeID, remindID,
	...
	public ArrayList<ScheduleVO> getAllSchedule() {
		...
		Cursor cursor = database.query("schedule_test", new String[] {
				"scheduleID", "scheduleTypeID", "remindID", "scheduleContent",
				"scheduleDate" }, null, null, null, null, "scheduleID desc");
		while (cursor.moveToNext()) {
			...
			String scheduleDate = cursor.getString(cursor
					.getColumnIndex("scheduleDate"));
			ScheduleVO vo = new ScheduleVO(scheduleID, scheduleTypeID,
					remindID, scheduleContent, scheduleDate);
			list.add(vo);
		}
		cursor.close();
	...
	public void delete(int scheduleID) {
		database.beginTransaction();
		try {
			database.delete("schedule_test", "scheduleID=?",
					new String[] { String.valueOf(scheduleID) });
			database.delete("scheduletagdate", "scheduleID=?",
					new String[] { String.valueOf(scheduleID) });
			database.setTransactionSuccessful();
		} finally {
			database.endTransaction();
	...
	public void update(ScheduleVO vo) {
		...
		database.update("schedule_test", values, "scheduleID=?",
				new String[] { String.valueOf(vo.getScheduleID()) });
	}
	 ...
	 * @return
	 */
	public ArrayList<ScheduleDateTag> getTagDate(int currentYear,
			int currentMonth) {
		ArrayList<ScheduleDateTag> dateTagList = new ArrayList<ScheduleDateTag>();
		Cursor cursor = database.query(
				"scheduletagdate",
				new String[] { "tagID", "year", "month", "day", "scheduleID" },
				"year=? and month=?",
				new String[] { String.valueOf(currentYear),
						String.valueOf(currentMonth) }, null, null, null);
		while (cursor.moveToNext()) {
			...
			int scheduleID = cursor.getInt(cursor.getColumnIndex("scheduleID"));
			ScheduleDateTag dateTag = new ScheduleDateTag(tagID, year, month,
					day, scheduleID);
	...
	public String[] getScheduleByTagDate(int year, int month, int day) {
		...
		Cursor cursor = database.query("scheduletagdate",
				new String[] { "scheduleID" }, "year=? and month=? and day=?",
