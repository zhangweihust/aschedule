...
public class HttpUtils implements Runnable {
    ...
    public static String doPost(Map<String, String> parmas, String url) {
        ...
        try {
            ...
            SharedPreferences sh = ScheduleApplication.getContext().getSharedPreferences(
                    UserInfoData.USER_INFO, ScheduleApplication.getContext().MODE_PRIVATE);

            String m_cookie = "";
            // m_cookie =ServiceManager.getSPUserInfo(UserInfoData.COOKIE);
            m_cookie = sh.getString(UserInfoData.COOKIE, "");
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                if (strResult.equals("-600")) {
                    ServiceManager.getEventservice().onUpdateEvent(
                            new EventArgs(EventTypes.COOKIE_ERROR));
                }
                return strResult;
            } else {
                for (int i = 0; i < 2; i++) {
